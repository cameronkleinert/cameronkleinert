...
function sendEmail() {
  if (!document.getElementsByClassName('contactme--email')[0].checkValidity()) {
    ...
    document.getElementsByClassName('contactme--feedback')[0].innerHTML =
      'Whoops!  Please enter a valid email address.';
  } else if (
    !document.getElementsByClassName('contactme--message')[0].checkValidity()
  ) {
    // $(".contactme--feedback").text("Whoops!  Please enter a message before sending.");
    document.getElementsByClassName('contactme--feedback')[0].innerHTML =
      'Whoops!  Please enter a message before sending.';
  } else {
    // depress button
    // $(".contactme--button").addClass("contactme--button--depressed");
    document
      .getElementsByClassName('contactme--button')[0]
      .classList.add('contactme--button--depressed');
    ...
    $.ajax('js/mail.php', {
      ...
      error: function error(a, b, c) {
        document.getElementsByClassName(
          'contactme--feedback',
        )[0].innerHTML = `Whoops!  Something went wrong.${b.toString()}; ${c.toString()}`;
      },
      complete: function complete() {
        // undepress button
        document
          .getElementsByClassName('contactme--button')[0]
          .classList.remove('contactme--button--depressed');
      },
    });
  }
}
